...
def test_decision_kernel_separates_countertrend_scalp_from_dominant_trend() -> None:
    ...
    assert int(result["countertrend_window_candles"]) in {1, 2}
    assert "countertrend" in str(result["market_conversation"]).lower()
...
def test_decision_kernel_marks_next_candle_trend_follow_when_micro_agrees() -> None:
    ...
    assert int(result["trend_follow_window_candles"]) >= 1
