...
def _post_frame(base_url: str, session_id: str, token: str, frame_id: int) -> dict[str, object]:
    fields = {
    ...
    body, content_type = _multipart_body(fields, "deployment_verify.png", _png_bytes())
    req = request.Request(
        f"{base_url.rstrip('/')}/v1/mobile/frame-ingest/sessions/{session_id}/frames",
        data=body,
        headers={
...
def main() -> int:
    ...
    if args.upload_smoke:
        if not str(args.token or "").strip():
            ...
            try:
                upload = _post_frame(base_url, str(args.session_id), str(args.token), 1)
