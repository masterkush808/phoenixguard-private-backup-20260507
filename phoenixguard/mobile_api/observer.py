...
def _signal_age_sec(signal: Mapping[str, Any]) -> float:
    completed_epoch = _safe_float(signal.get("completed_epoch", 0.0), 0.0)
    if completed_epoch <= 0.0:
        return 0.0
    return max(0.0, _now_epoch() - completed_epoch)
...
class SignalObserverService:
    ...
    def _render_signal(
        ...
        stale = bool(age_sec >= stale_after_sec or freshness < min_freshness)
