...
- Keep secrets in `Backend\launch\deploy\windows\phoenixguard.vm-share.env.ps1`
...
1. Copy `Backend\launch\deploy\windows\phoenixguard.vm-share.env.example.ps1` to `Backend\launch\deploy\windows\phoenixguard.vm-share.env.ps1`.
...
.\Backend\launch\deploy\windows\Start-PhoenixGuardVmShare.ps1 -Bootstrap
```
...
```powershell
cd C:\PhoenixGuard\phoenixguard
.\Backend\launch\deploy\windows\Register-PhoenixGuardShareTask.ps1 -BootstrapOnFirstRun -StartNow
```
...
.\Backend\launch\deploy\windows\Setup-PhoenixGuardCloudflare.ps1 -WriteConfig -ConfigureAccess -InstallService
```
...
```powershell
cd C:\PhoenixGuard\phoenixguard
.\Backend\launch\deploy\windows\Install-CloudflaredTunnel.ps1 -TunnelToken 'eyJ...'
```
...
```powershell
cd C:\PhoenixGuard\phoenixguard
.\Backend\launch\deploy\windows\Start-PhoenixGuardQuickTunnel.ps1
```
...
- writes logs and the discovered URL under `Backend\launch\deploy\windows\logs`

To stop the temporary Quick Tunnel:

```powershell
cd C:\PhoenixGuard\phoenixguard
.\Backend\launch\deploy\windows\Stop-PhoenixGuardQuickTunnel.ps1
```
