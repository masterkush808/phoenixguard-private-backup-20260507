...
class OpticalFlowTracker:
    ...
    def __init__(
        ...
        # DIS optical flow detector. FAST keeps live frame-to-frame tracking under
        # budget while the higher-level tracker supplies candle/zone semantics.
        cv2_dynamic = cast(Any, cv2)
        self.flow_detector = cv2_dynamic.DISOpticalFlow_create(cv2.DISOPTICAL_FLOW_PRESET_FAST)
    ...
    def process_frame(
        ...
        if self.prev_gray is None:
            self.prev_gray = gray
            flow_frame = self._neutral_frame(gray, timestamp_ms)
        ...
        if gray.shape == self.prev_gray.shape and np.array_equal(gray, self.prev_gray):
            flow_frame = self._neutral_frame(gray, timestamp_ms)
            self.frame_history.append(flow_frame)
            if len(self.frame_history) > self.accumulation_window:
                self.frame_history.pop(0)
    ...
    def _neutral_frame(self, gray: NDArray[np.uint8], timestamp_ms: float) -> OpticalFlowFrame:
        h, w = gray.shape
