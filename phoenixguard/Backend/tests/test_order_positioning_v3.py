...
def _session(
    ...
    **overrides: Any,
    ...
    payload: dict[str, Any] = {
        ...
        "reaction_window_horizon_steps": visible_horizon,
...
def test_buy_builds_pullback_breakout_and_distinct_protective_stop_areas() -> None:
    ...
    assert limit[0]["bounds"] == [0.72, 0.533333, 0.88, 0.633333]
    ...
    assert stop_entry[0]["bounds"] == [0.72, 0.160667, 0.88, 0.166667]
...
def test_reaction_window_uses_the_available_visible_horizon_from_one_to_thirty_two() -> None:
    for horizon in (1, 8, 16, 24, 32):
        candidate = build_order_positioning_candidates_v3(
            _session(
                overlays=[_demand()],
                visible_horizon=horizon,
                reaction_window_origin_x_norm=0.50,
                reaction_window_step_x_norm=0.01,
            )
        )

        assert candidate["status"] == "READY"
        assert candidate["horizon_steps"] == horizon
        assert candidate["reaction_window"]["horizon_steps"] == horizon
        assert candidate["reaction_window"]["x_bounds"] == [
            0.5,
            round(0.5 + horizon * 0.01, 6),
        ]
        assert all(
            zone["bounds"][0] == 0.5
            and zone["bounds"][2] == round(0.5 + horizon * 0.01, 6)
            for zone in candidate["candidate_zones"]
        )

    for invalid_horizon in (0, 33):
        blocked = build_order_positioning_candidates_v3(
            _session(overlays=[_demand()], visible_horizon=invalid_horizon)
        )
        assert blocked["status"] == "BLOCKED"
        assert blocked["candidate_zones"] == []
        assert blocked["blockers"] == ["REACTION_WINDOW_GEOMETRY_INVALID"]
...
def test_non_overlapping_same_intent_selects_nearest_route_before_protection() -> None:
    ...
    assert entries[0]["bounds"] == [0.72, 0.483333, 0.88, 0.55]
