...
def test_run_memory_projection_resolves_adapter_artifacts(tmp_path: Path) -> None:
    ...
    public_result = svc.run_memory_projection(session_id, mode="future")
    assert "memory_projection_future" not in public_result
    assert "memory_projection_current" not in public_result
    mem = svc.require_session(session_id)["memory_projection_future"]
...
def test_retired_show_future_and_projection_artifact_routes_are_absent(
    tmp_path: Path,
) -> None:
    svc = ContinuousWindowTrackerService(root_dir=tmp_path / "wt", tracking_adapter=FakeAdapter())
    session_id = "s_api"
    ...
    img.save(chart_path)
    artifact_dir = svc.session_dir(session_id) / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    private_projection = artifact_dir / "private_predict_projection.png"
    private_reference = artifact_dir / "private_memory_reference.png"
    img.save(private_projection)
    img.save(private_reference)
    payload: dict[str, Any] = {
        ...
        "memory_projection_predict": {
            "status": "ready",
            "mode": "predict",
            "projection_image_path": str(private_projection),
            "reference_image_path": str(private_reference),
        },
    }
    svc.save_session(payload)
    ...
    assert client.get(
        f"/v1/mobile/window-tracker/sessions/{session_id}"
        f"/artifacts/files/{private_projection.name}"
    ).status_code == 404
    assert client.get(
        f"/v1/mobile/window-tracker/sessions/{session_id}"
        f"/artifacts/files/{private_reference.name}"
    ).status_code == 404
    private = svc.require_session(session_id)
    assert private.get("memory_projection_active_mode") == "predict"
    private_projection_state = private.get("memory_projection_predict", {})
    assert private_projection_state.get("status") in {"ready", "stale"}
    assert private_projection_state.get("projection_image_path") == str(
        private_projection
    )


def test_retired_predict_and_forecast_action_routes_are_absent(tmp_path: Path) -> None:
    svc = ContinuousWindowTrackerService(root_dir=tmp_path / "wt", tracking_adapter=FakeAdapter())
    ...
    try:
        ...
        for retired_name in (
            ...
            "get_memory_projection_action",
            "_run_memory_projection_action",
            "_forecast_action_executor",
            "_forecast_actions",
            "_forecast_action_futures",
            "_forecast_action_keys",
            "_latest_forecast_action",
            "_forecast_action_created_epoch",
            "_forecast_action_shutdown",
        ):
            assert not hasattr(svc, retired_name)
    finally:
        svc.shutdown()
