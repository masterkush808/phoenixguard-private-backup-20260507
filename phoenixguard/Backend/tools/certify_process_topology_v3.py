...
def main() -> int:
    ...
    if len(tracker_processes) != 1:
        ...
        corrections.append(
            r"Start tracker with: .\.venv-live\Scripts\python.exe Backend\launch\start_phoenixguard_24_7_tracker.py --host 127.0.0.1 --port 8793 --session-id pocket-live-8788 --capture-interval 30 --no-open-dashboard"
        )
    ...
    if not shooter_processes and not args.allow_missing_shooter:
        ...
        corrections.append(
            r"Start shooter with: .\.venv-live\Scripts\python.exe Backend\launch\shooter.py signal --base-url http://127.0.0.1:8793 --session-id pocket-live-8788 --poll 30.0 --heartbeat 4.0"
        )
