...
if (Test-Path -LiteralPath $PidFile) {
    throw "Mock business stack already has a PID file at $PidFile. Run .\scripts\start_business_mock_local.ps1 -Stop first."
}
...
if (-not $SkipApi) {
    $apiCommand = "python -m uvicorn scripts.business_mock_api:app --host $ApiHost --port $ApiPort --log-level info"
...
if (-not $SkipWeb) {
    if (-not (Test-Path -LiteralPath (Join-Path $WebDir "package.json"))) {
        throw "web/package.json was not found. Create or fetch the mock Next.js app before starting the web process."
    }
...
Write-Host "Run E2E:"
Write-Host "Push-Location web; `$env:BUSINESS_E2E='1'; `$env:BUSINESS_WEB_BASE_URL='$WebBaseUrl'; `$env:BUSINESS_API_BASE_URL='$ApiBaseUrl'; npx playwright test tests/e2e/business-mock-flow.spec.ts --reporter=line; Pop-Location"
Write-Host ""
Write-Host "Stop:"
Write-Host ".\scripts\start_business_mock_local.ps1 -Stop"
