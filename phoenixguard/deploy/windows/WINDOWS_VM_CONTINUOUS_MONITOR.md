...
Copy-Item .\deploy\windows\phoenixguard.vm-monitor.env.example.ps1 .\deploy\windows\phoenixguard.vm-monitor.env.ps1
notepad .\deploy\windows\phoenixguard.vm-monitor.env.ps1
```
...
```powershell
cd C:\PhoenixGuard\phoenixguard
.\deploy\windows\Start-PhoenixGuardVmMonitor.ps1 -Bootstrap
```
...
```powershell
cd C:\PhoenixGuard\phoenixguard
.\deploy\windows\Register-PhoenixGuardVmMonitorTask.ps1 -BootstrapOnFirstRun -StartNow
```
...
```powershell
python shooter.py signal --session-id pocket-live-8788 --base-url http://VM_PRIVATE_IP:8793 --poll 1.0 --preferred-source tracker --require-preferred-source --min-confidence 0.58 --max-signal-age 8
```
