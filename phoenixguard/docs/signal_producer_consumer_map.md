...
| `phoenixguard/runtime/model_council_daemon.py` | Prediction response from local ensemble runtime | `/status`, `/predict` | Needs V3 arbitration/publisher layer |
| `phoenixguard/decision/model_council_v3.py` | Council result and optional `execution_packet` / `model_council_packet` | In-process evaluator/publisher | Uses Agent 1 packet helpers |
| `phoenixguard/execution/packet_v3.py::build_execution_packet_v3` | `PG_EXECUTION_PACKET_V3` packet | New reusable support module | Ready for integration |
...
| `scripts/replay_signals.py` | Raw shooter parse payloads | Should be migrated to V3 packet replay |
| `window_tracker_dashboard.html` | Tracker session `latest_signal` | Diagnostic only under V3 |
| `tests/test_shooter_runtime.py` and `tests/test_shooter_parse.py` | Legacy shooter behavior | Must be updated by Agent 4 |
| `tests/test_execution_packet_schema_v3.py` | New V3 schema module | Agent 1 coverage |
...
- `phoenixguard/execution/governor.py::validate_fire_command` still treats `SNIPER_READY`, `TRIGGER_READY`, `READY_TO_FIRE`, and similar states as armed.
- `phoenixguard/decision/decision_kernel.py` promotes state when `entry_state` is `TRIGGERED`, `TRIGGER_READY`, or `SNIPER_READY`, or when `execution_action` equals dominant side.
...
- `phoenixguard/mobile_api/window_tracker.py` still builds diagnostic execution lanes and mutates `latest_signal["expiry_seconds"]`, `latest_signal["execution_lane"]`, and `latest_signal["execution_timing"]`; in `execution_mode=live` it now refuses internal clicks and defers valid V3 packets to the standalone shooter.
