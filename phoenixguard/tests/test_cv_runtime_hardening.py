...
def test_hf_bootstrap_stays_cache_only_until_explicitly_enabled(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cv_module, "YOLOModel", lambda path: path)
    monkeypatch.setattr(cv_module, "hf_hub_download", _fake_download)
    monkeypatch.setattr(cv_module, "can_import_torchvision_safely", lambda: True)
    monkeypatch.setattr(CVPatternDetector, "_load_or_train_memory_classifier", lambda self: None)
...
def test_detect_uses_heuristic_fallback_when_raw_backend_is_empty(monkeypatch) -> None:
    monkeypatch.setattr(CVPatternDetector, "_try_load_hf_yolo_weights", lambda self, model_ref: False)
    ...
    monkeypatch.setattr(detector, "_raw_detect", lambda image_rgb: [])
    monkeypatch.setattr(
        detector,
        "_heuristic_candle_detect",
        lambda image_rgb: [
...
def test_detect_returns_heuristic_structure_without_loaded_yolo(monkeypatch) -> None:
    monkeypatch.setattr(CVPatternDetector, "_try_load_hf_yolo_weights", lambda self, model_ref: False)
    ...
    monkeypatch.setattr(detector, "_heuristic_candle_detect", lambda image_rgb: list(heuristic_rows))
...
def test_optional_grounded_parser_uses_cache_only_bootstrap_by_default(monkeypatch) -> None:
    calls: list[dict[str, object]] = []
    ...
    class _FakeLoader:
        ...
        def from_pretrained(self, model_name: str, **kwargs) -> _FakeLoaded:  # type: ignore[no-untyped-def]
