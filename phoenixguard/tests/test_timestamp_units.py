...
def test_s2_execution_packet_expires_after_valid_until_epoch_sec() -> None:
    payload = packet(valid_until_epoch=NOW + 2.0, valid_until_epoch_sec=NOW - 0.001)
...
def test_s2packet_age_ms_uses_seconds_epoch_correctly() -> None:
    payload = packet(created_epoch_sec=NOW - 1.25, created_epoch=NOW - 1.25)

    assert packet_age_ms(payload, now_epoch=NOW) == 1250


def test_s2_millisecond_epoch_is_rejected() -> None:
    payload = packet(created_epoch_sec=NOW * 1000.0, created_epoch=NOW * 1000.0)
