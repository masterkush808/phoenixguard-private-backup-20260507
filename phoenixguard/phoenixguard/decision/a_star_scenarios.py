...
===========================================
Ranks candidate candle paths using A* search without rendering synthetic live candles.
...
class ScenarioNode:
    ...
    def last_candle(self) -> CandleState | None:
...
class A_StarScenarioPredictor:
    ...
    def _node_signature(self, node: ScenarioNode) -> tuple[int, float, float, str]:
        """Hashable signature for deduplication."""
        last = node.last_candle()
        if last is None:
            return (node.depth, 0.0, 0.0, "HOLD")
        return (
            node.depth,
            round(last.open, 4),
            round(last.close, 4),
