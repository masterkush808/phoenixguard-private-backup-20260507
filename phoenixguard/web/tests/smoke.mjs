...
const screenshotDir = path.join(repoRoot, ".codex_runtime", "web-smoke");
...
async function startServerIfNeeded() {
  ...
  const hasProductionBuild = await fileExists(path.join(webRoot, ".next", "BUILD_ID"));
  const mode = process.env.WEB_SMOKE_NEXT_MODE || (hasProductionBuild ? "start" : "dev");
  const child = spawn(
    process.execPath,
